...
Full pass (post-M7), on `feature/multilingual`: the site went from "French strings hardcoded everywhere" to genuinely trilingual (fr default, en, ar) via **Polylang (free)**. Arabic gets `dir="rtl"` at `<html>` automatically (`is_rtl()` via `language_attributes()`), and all custom CSS already used logical properties (`inline-start/end`, `margin-inline-*`) instead of `left/right`, so RTL mirroring needed no second hand-maintained stylesheet.

**Content**: language taxonomy enabled for `post`, `page`, `product`, `product_cat`. All WooCommerce pages (Shop/Cart/Checkout/My Account), the 6 product categories, and all 10 products (with their size/color variations and images reused, not re-uploaded) were duplicated into EN and AR via one-off admin scripts (not committed — see the session's git log for the pattern). Static pages (About/Contact/Terms/Privacy) likewise.

**UI strings**: ~172 theme strings translated by hand into EN/AR and compiled to `wp-content/themes/hasna-shoes/languages/hasna-shoes-{en_US,ar}.mo`. WordPress core and the WooCommerce plugin's own official EN/AR/FR language packs were installed separately so core/plugin strings ("Showing N results", checkout field labels, etc.) aren't left in English.

**Header language switcher**: `pll_the_languages()` in `header.php`/`footer.php`, degrades gracefully (hidden) if Polylang isn't active.

### Integration gaps free Polylang doesn't cover — all fixed in `functions.php` / `inc/polylang-woocommerce.php`

Free Polylang makes posts/terms language-aware, but does **not** make WooCommerce itself multilingual (that's specifically what the paid "Polylang for WooCommerce" add-on does). Getting a fully-correct EN/AR checkout required working around several gaps:

1. **Textdomain loaded before Polylang knows the language.** `load_theme_textdomain()` on `after_setup_theme` always resolved the *default* (French) locale, because Polylang determines the request's language from the URL *after* `after_setup_theme` runs — and WordPress's "just in time" textdomain autoloading (triggered by the first `__()` call, e.g. `register_nav_menus()` labels) then permanently marks the domain as loaded for the rest of the request, so a later correctly-timed reload was silently a no-op. **Fix**: moved loading to the `wp` hook with an explicit `unload_textdomain()` before `load_textdomain()`.
2. **`wc_get_page_id()` (shop/cart/checkout/my-account) isn't language-aware.** Filtered `option_woocommerce_{shop,cart,checkout,myaccount}_page_id` and `option_wp_page_for_privacy_policy` to resolve to the current language's translation via `pll_get_post()`.
3. **Checkout AJAX fragments (order review table, COD description) rendered in French regardless of page language.** `WC_AJAX::get_endpoint()` always builds the `wc-ajax` URL from the *default-language* `home_url()`, never the current page's `/en/`-prefixed URL — so `update_order_review` (which WooCommerce fires automatically on checkout page load) was a same-origin request Polylang resolved as French. **Fix**: filtered `woocommerce_ajax_get_endpoint` to rebuild it from `pll_home_url( pll_current_language() )`.
4. **Several WooCommerce strings are raw option values, never passed through gettext**, so they don't translate with everything else: the COD gateway title (`woocommerce_gateway_title` filter, mapped per language) and the checkout/registration privacy-policy notice (`option_woocommerce_checkout_privacy_policy_text` / `..._registration_privacy_policy_text`, mapped per language — this one was in English even on the *French* site before this fix, since WooCommerce's own default value is English and was never localized).
5. **Product attribute labels** (`pa_taille` → "Taille", `pa_couleur` → "Couleur") are a single global taxonomy value, not per-language — mapped via `woocommerce_attribute_label`.
6. **The Shop page's EN/AR translations have no product-archive rewrite rule.** WordPress registers the `product` post type's archive rewrite (`has_archive`) once, from the *default-language* Shop page's slug — so `/shop/` (French) genuinely matches a `post_type=product` archive query, but `/en/shop-2/` just matches the ordinary WP Page rewrite and resolves to a single Page post. WooCommerce still *selects* `archive-product.php` as the template (`is_shop()` only checks `is_page( wc_get_page_id( 'shop' ) )`, which passes), so the correct template rendered around a genuinely empty product loop — both the category filter chips and the product grid silently disappeared on the translated Shop pages. **Fix**: a `pre_get_posts` callback (priority **5**, i.e. before `WC_Query::pre_get_posts()` at the default priority 10) detects when the queried page is a Polylang translation of the default-language Shop page and swaps the query to a real product archive itself, so WooCommerce's own `product_query()` setup (tax query, ordering, per-page, the `wc_query` flag `wc_setup_loop()` needs for `wc_get_loop_prop('total')`) runs normally afterward. `page_id`/`pagename` aren't resolved to a post ID yet at `pre_get_posts` time, so the page is identified via `get_page_by_path( $q->get( 'pagename' ) )`.
7. **Translated products were tagged with the French-language category terms**, not their per-language equivalents (Polylang duplicates each `product_cat` term per language with its own `term_id`) — so English/Arabic category term counts were 0 and category archives/shop filter chips showed no products for those languages even after fix #6. Fixed via a one-off data migration remapping each translated product's terms through `pll_get_term()`.
8. **`get_term_by( 'slug', ... )` is silently scoped to the current language by Polylang.** Every place the theme looked up a category by its fixed French slug (`hasna_primary_nav_items()`'s "Nouveautés" link, `hasna_category_url()` used by the footer, `hasna_homepage_categories()` used by the homepage category tiles) returned nothing on the EN/AR site and silently fell back to a broken link. Fixed by passing `'lang' => 'fr'` explicitly on those specific lookups (the slug is always French, regardless of which language is currently being viewed).
9. **Site tagline** (`blogdescription`, feeds the front-page `<title>` and SEO meta) stayed French on every language — mapped per language via `option_blogdescription` (with an `is_admin()` guard so Settings → General still shows/edits the real stored value).

### Free vs. paid Polylang — one accepted limitation

Free Polylang does **not** allow the same slug to be reused across languages (translating slugs, e.g. `/en/checkout/` instead of `/en/checkout-2/`, is a Polylang Pro feature). WordPress auto-suffixes colliding slugs instead (`checkout` → `checkout-2` → `checkout-3`, `shop` → `shop-2` → `shop-3`, etc.). This is expected, not a bug — every URL in this codebase is generated dynamically (`wc_get_checkout_url()`, `get_permalink()`, `pll_get_post()` + `get_permalink()`, never hardcoded), so the suffixed slugs resolve correctly everywhere without any special-casing.

### Verification

Manually walked, per language (fr/en/ar): homepage, shop archive, a category archive, a product page, cart, and a full guest Cash-on-Delivery checkout through to "Order received" (an English test order was placed, confirmed correct, then deleted). Confirmed via `wp-content/debug.log` line-count diffing that none of the above introduced new PHP notices/errors.
